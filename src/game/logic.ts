import {
  ...
  RAW_MAP, BLOCKED, isWalkable, getDanger,
  ENEMY_TEMPLATES, CLASSES, T,
} from './constants';
import type { GameState, Hero, Enemy, Particle, FloatingText, BattleState } from './types';
...
function makeBattleState(): BattleState {
  return { frozen: 0, poisoned: 0, evading: false, shieldUp: false, buffAtk: 0, buffAtkT: 0, blessed: false, blessT: 0 };
}
...
export function startBattle(state: GameState, enemyId: number) {
  ...
  state.log = [...state.log, { text: `⚔ ${state.enemies.find(e => e.id === enemyId)?.name} apareceu!`, type: 'sys' }];
}
...
export function endBattle(state: GameState, won: boolean) {
  ...
  if (!won) {
    ...
    addFloat(state, 'MORT!', hero.px, hero.py, '#e04040');
  }
}
...
export function resolveSkill(
  ...
  const totalAtk = hero.atk + (b.buffAtkT > 0 ? b.buffAtk : 0) + (b.blessed && b.blessT > 0 ? 3 : 0);
  let skipEnemy = false;
  ...
  if (['phys', 'magic', 'holy'].includes(sk.type)) {
    ...
    if (crit) { dmg = Math.floor(dmg * 2); addLog(`⚡ CRÍTICO! ${sk.name} -${dmg} HP`, 'hit'); addFloat(state, 'CRIT -' + dmg, enemy.tx, enemy.ty, '#ffe050'); }
    else { addLog(`${sk.name}: -${dmg} HP`, 'dmg'); addFloat(state, '-' + dmg, enemy.tx, enemy.ty, '#ff5040'); }
    enemy.hp -= dmg;
    ...
    if (enemy.hp <= 0) { resolveEnemyDeath(state, enemy); return { enemyTurnCallback: null }; }
    ...
    let dmg = Math.max(1, Math.floor(totalAtk * sk.mult) - Math.floor(enemy.def * 0.3) + rnd(-1, 2));
    enemy.hp -= dmg;
    b.poisoned = 4;
    ...
    let dmg = Math.max(1, Math.floor(totalAtk * sk.mult) - Math.floor(enemy.def * 0.3) + rnd(-1, 3));
    enemy.hp -= dmg;
    b.frozen = 2;
    skipEnemy = true;
  ...
  if (skipEnemy) return { enemyTurnCallback: null };
  return { enemyTurnCallback: () => resolveEnemyTurn(state) };
}

export function resolveEnemyTurn(state: GameState) {
  const hero = state.hero!;
  const enemy = state.enemies.find(e => e.id === state.battleEnemyId);
  ...
  if (b.poisoned > 0) {
    ...
    if (enemy.hp <= 0) { resolveEnemyDeath(state, enemy); return; }
  }
  ...
  const effDef = hero.def + (b.shieldUp ? hero.def : 0) + (b.blessed && b.blessT > 0 ? 3 : 0);
  b.shieldUp = false;
...
function resolveEnemyDeath(state: GameState, enemy: Enemy) {
  const hero = state.hero!;
  state.log = [...state.log.slice(-60), { text: `✓ ${enemy.name} derrotado! +${enemy.exp} XP +${enemy.gold} ouro`, type: 'exp' }];
  hero.exp += enemy.exp;
...
function checkLevelUp(state: GameState) {
  ...
  while (hero.exp >= hero.expNext) {
    ...
    state.log = [...state.log.slice(-60), { text: `⬆ NÍVEL ${hero.level}! HP e MP restaurados!`, type: 'exp' }];
  }
}
