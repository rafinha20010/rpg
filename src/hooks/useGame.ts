...
export function useGame(canvasRef: React.RefObject<HTMLCanvasElement | null>) {
  ...
  useEffect(() => {
    const onDown = (e: KeyboardEvent) => {
      ...
      if (e.code === 'Space' && s.inBattle) { doSkill(0); }
      if (e.code === 'Digit1' && s.inBattle) doSkill(0);
      if (e.code === 'Digit2' && s.inBattle) doSkill(1);
      if (e.code === 'Digit3' && s.inBattle) doSkill(2);
      if (e.code === 'Digit4' && s.inBattle) doSkill(3);
    };
  ...
  const doSkill = useCallback((idx: number) => {
    ...
    tick();
    if (s.hero.hp <= 0) {
      setTimeout(() => { endBattle(s, false); tick(); }, 800);
    } else if (enemyTurnCallback) {
      setTimeout(() => { enemyTurnCallback(); tick(); }, 560);
    }
  }, []);
  ...
  const doFleeFn = useCallback(() => {
    ...
    const fled = doFlee(s);
    if (!fled) {
      setTimeout(() => { resolveEnemyTurn(s); tick(); if (s.hero!.hp <= 0) { endBattle(s, false); tick(); } }, 500);
    }
    tick();
  }, []);
